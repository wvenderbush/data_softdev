...
class _Constants(object):
    ...
    '''
    _HEADER = {'User-Agent': 
              'CORGIS State Fragility library for educational purposes'}
...
_Constants._DATABASE_NAME = _os.path.join(_os.path.dirname(_os.path.realpath(__file__)), "state_fragility.db")
...
def get_scores(test=False):
    ...
    if _Constants._TEST or test:
        rows = _Constants._DATABASE.execute("SELECT data FROM fragility LIMIT {hardware}".format(
        ...
        rows = _Constants._DATABASE.execute("SELECT data FROM fragility".format(
            hardware=_Constants._HARDWARE))
        data = [r[0] for r in rows]
        data = [_Auxiliary._byteify(_json.loads(r)) for r in data]
